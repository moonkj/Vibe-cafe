...
#### 보안 강화 (SQL / Dart)
- [x] `032_update_spot_after_report_security.sql`: `p_user_id` 제거 → `auth.uid()` 서버 검증, dB 범위 0~120 체크
- [x] `admin_config.dart`: 관리자 UUID `dart-define` 주입 (`String.fromEnvironment`)
- [x] `app_router.dart`: `record_user_session` RPC를 `_sessionRecorderProvider`로 분리 (단일 책임)
- [x] `report_repository.dart`: `update_spot_after_report` 호출에서 `p_user_id` 제거

#### 안정성 / 버그 수정
- [x] `map_controller.dart`: `onDispose`에 `mapController = null` 추가 (null-safe dispose)
- [x] `map_controller.dart`: `_discoverNearbyCafes` 좌표 범위 검증 — 반경 밖 Places 결과 제외
- [x] `map_controller.dart`: `_discoverNearbyCafes` 자동 `_loadSpots` 제거 → 탭 전환 중 마커 급작스러운 출현 방지
- [x] `report_controller.dart`: `_disposed` 플래그 추가 — GPS 스트림 비동기 경쟁 조건 방지
- [x] `badge_service.dart`: XP 지급 실패 시 `debugPrint` 로깅 추가
- [x] `033_get_spots_near_limit.sql`: `get_spots_near` LIMIT 200 복원

#### 성능 최적화
- [x] `map_screen.dart`: `_spotBitmapCache` 모듈 레벨 전역화 — 탭 전환 시 200개 비트맵 재생성 제거
- [x] `map_screen.dart`: 비트맵 생성 순차 `await` → `Future.wait` 병렬화
- [x] 캐시 크기 100 → 150 확대

#### UI / 코드 품질
- [x] `report_screen.dart`: 하드코딩 색상 → `onSurface.withValues(alpha: 0.5)` 테마색 적용
- [x] `settings_screen.dart`, `report_screen.dart`, `my_map_screen.dart`: `AppBackButton` / `AppLoading` 교체

**결과**: `flutter analyze` 0 issues, 지도탭 마커 로딩 체감 대폭 개선
